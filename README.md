...
> 规则引擎基线:整体准确率 84.5%,三语种间已有差异。真正弱点是口语化表达(如"有点冷,温度升到 26 度"等),LLM 路由正是为此设计——下一步验证目标是三语种均达 90%+。
...
### 4. 交互文档查询(PM 日常工具) — 缺陷文本自动定位到交互文档功能,判定覆盖度

![交互文档查询](docs/screenshots/04_spec_lookup.png)

> 输入缺陷描述 → 自动分类意图 → 在交互文档里查找对应功能 → 输出命中位置 + 覆盖度判定(完整/部分/未定义)。把人工翻文档定位的时间从平均 5 分钟降至 <10 秒,并能识别"文档定义不全"的产品规格盲点。
...
- **质量看板**：跑评测集，按 `中文 / 英文 / 混说` 三个维度看准确率，直观暴露口语化表达的挑战。
- **交互文档查询**(PM 日常工具)：输入缺陷描述，自动分类意图并在交互文档(xlsx)里查找对应功能，判定覆盖度(完整/部分/未定义)——将人工翻文档定位的时间从 5 分钟降至 <10 秒。
